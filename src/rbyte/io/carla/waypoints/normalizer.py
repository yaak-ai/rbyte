...
class Columns(BaseModel):
    ego_coordinates: str | tuple[str, str]
...
class CarlaWaypointNormalizer:
    ...
    def _normalize(self, input: pl.DataFrame) -> pl.DataFrame:
        ...
        match self._columns.ego_coordinates:
            case str():
                # or the other way around
                x_ego, y_ego = (
                    input.select(
                        pl.col(self._columns.ego_coordinates).arr.get(i).repeat_by(10).explode()
                    )
                    .to_series()
                    .to_numpy()
                    for i in range(2)
