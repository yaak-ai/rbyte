...
class WaypointBuilder:
    ...
    @validate_call
    def __init__(self, *, input_column: str, output_column: str, length: int) -> None:
        self._length = length
    ...
    def _build(self, input: pl.DataFrame) -> pl.DataFrame:
        lf = input.lazy()
        srid = (
            lf.select(st.geom(self._input_column).st.srid().unique()).collect().item()
        )
    ...
    def _normalize(self, input: pl.DataFrame) -> pl.DataFrame:
        ...
        lf = (
            lf.with_columns(
                ...
                .st.rotate(angle=pl.col(self._columns.heading))
            )
